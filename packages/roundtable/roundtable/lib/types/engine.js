...
import { RoundtableEngine, RoundtableError } from "./service.js";
...
import { createHostSummarizer, renderDriverError } from "./driver.js";
/**
...
export class RoundtableEngineProvider extends RoundtableEngine {
    ...
    static Config = z.object({ provider: z.string().default('spawn'), maxMembers: z.number().default(8) });
    provider;
    maxMembers;
    recorder;
    ...
    start(request) {
        ...
        if (request.members.length > this.maxMembers)
            throw new RoundtableError(`roster exceeds maxMembers (${this.maxMembers})`, 'ROSTER_INVALID');
        const ids = new Set(request.members.map(m => m.id));
        if (ids.size !== request.members.length)
        ...
        if (llm !== undefined) {
            ...
            for (const member of request.members) {
                ...
                if (provider !== undefined && !available.has(provider)) {
                    throw new RoundtableError(`member "${member.id}" routes to provider "${provider}" which is not registered`
                        + ` (available: ${[...available].join(', ') || 'none'})`, 'ROSTER_INVALID');
                }
            }
        }
        ...
        const summarize = request.synthesize === false
            ? async () => []
        ...
        const result = (async () => {
            try {
                ...
                const discussion = { id, roster: request.members, rounds: [minutes], status: 'completed' };
                ...
                return { stopReason: 'completed', agentsStarted: request.members.length, rounds: settledRounds };
            }
            catch (error) {
                if (signal.aborted) {
                    this.emitRoundtableEvent('roundtable/end', info, 'cancelled');
                    this.recorder.finish(id, 'cancelled');
                    return { stopReason: 'cancelled', agentsStarted: membersStarted, rounds: settledRounds };
                }
                ...
                return { stopReason: 'error', agentsStarted: membersStarted, rounds: settledRounds };
            }
        })();
        return {
            id,
...
export function createRoundtableEngine(ctx, deps) {
