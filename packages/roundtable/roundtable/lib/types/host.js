...
import { createRoundtableHostDriver, parseGateReply, } from "./driver.js";
 ...
 */
function parseStartPayload(rawInput) {
    let value;
    try {
    ...
    for (const entry of membersRaw) {
        ...
        const parsed = { id: id.trim(), label: label.trim() };
        ...
        if (member.agentOptions !== undefined) {
            ...
            const options = member.agentOptions;
            const agentOptions = {};
            ...
            if (Object.keys(agentOptions).length > 0)
                parsed.agentOptions = agentOptions;
        }
        members.push(parsed);
    }
 ...
 */
export function renderNextRoundPrompt(minutes) {
...
function restoreClaimed(agent, messages, excluded) {
    for (const message of messages.toReversed()) {
        if (excluded !== undefined && message.id === excluded.id)
            continue;
...
export function apply(ctx, config) {
    ...
    function stateFor(agent) {
        ...
        if (existing !== undefined)
            return existing;
        const state = { agent, driver: undefined, discussions: new Map(), stopping: false };
    ...
    function presentNextRound(state, discussionId, minutes) {
        if (state.stopping)
            return;
        if (!state.discussions.has(discussionId))
            return;
    ...
    function startDiscussion(request) {
        const state = stateFor(request.host);
        if (state.discussions.size > 0) {
            throw new RoundtableError(`roundtable-host: a discussion is already active on agent "${request.host.id}"`
                + ` (${[...state.discussions.keys()][0]}); the host loop is single-active — stop or cancel`
                + ' it before starting another', 'DISCUSSION_ACTIVE');
        }
    ...
    function cancelDiscussion(host, id) {
    ...
    ctx.effect(function* () {
        ...
        ctx.on('agent/disposed', ({ agent }) => {
            ...
            if (state === undefined)
                return;
            state.stopping = true;
            for (const id of state.discussions.keys())
                state.driver.cancelDiscussion(id);
            ...
            if (handle.phase === 'awaitingGate') {
                ...
                if (gate !== undefined) {
                    if (gate.gate === 'continue') {
                        ...
                        void state.driver.continueRound(discussionId, opinion).catch((error) => {
