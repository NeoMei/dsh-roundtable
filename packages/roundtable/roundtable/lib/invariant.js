...
const name = "roundtable-invariant";
...
function validateStart(value, fail) {
	...
	if (!Array.isArray(roster) || roster.length === 0) fail("roundtable/start roster must be a non-empty array");
	else for (const member of roster) {
...
function validateMinutes(value, fail) {
	...
	if (typeof roundNumber !== "number" || !Number.isInteger(roundNumber) || roundNumber < 1) fail("roundtable/round-end minutes roundNumber must be a positive integer");
	if (typeof topic !== "string") fail("roundtable/round-end minutes topic must be a string");
	for (const key of [
		"utterances",
		"humanSteers",
		"summary"
	]) if (!Array.isArray(value[key])) fail(`roundtable/round-end minutes ${key} must be an array`);
}
...
function validateEvent(event, fail) {
	switch (event.type) {
		...
		case "roundtable/round-end": {
			...
			if (typeof data.discussionId !== "string" || data.discussionId.length === 0) fail("roundtable/round-end discussionId must be a non-empty string");
			validateMinutes(data.minutes, fail);
			break;
		}
		case "roundtable/end": {
			const data = event.data;
			if (typeof data.discussionId !== "string" || data.discussionId.length === 0) fail("roundtable/end discussionId must be a non-empty string");
			if (typeof data.stopReason !== "string" || !STOP_REASONS.has(data.stopReason)) fail(`roundtable/end stopReason must be one of ${[...STOP_REASONS].join(", ")}`);
			break;
		}
		default: break;
	}
