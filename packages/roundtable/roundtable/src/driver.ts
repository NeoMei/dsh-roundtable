...
import { recoverRoundtableDiscussions } from './recovery.ts'
import { RoundtableId } from './types.ts'
import type {
  RoundMinutes, RoundtableDiscussion, RoundtableInfo,
  RoundtableMember, RoundtableStopReason,
} from './types.ts'
...
const STOP_WORDS = ['停止讨论', '停止', '结束', '终止', 'stop', 'end', 'quit'] as const
/** Command words recognized as "continue" (longer first so "继续讨论" wins over "继续"). */
const CONTINUE_WORDS = ['继续讨论', '下一轮', '继续', 'continue', 'next'] as const
 ...
 */
export function parseGateReply(text: string): RoundtableGateReply | undefined {
  const trimmed = text.trim()
  if (trimmed === '') return undefined
  const lower = trimmed.toLowerCase()
  const lead = (word: string): string | undefined => {
  ...
  for (const word of STOP_WORDS) {
    ...
    if (opinion !== undefined) return { gate: 'stop', opinion }
  }
  for (const word of CONTINUE_WORDS) {
    const opinion = lead(word)
    if (opinion !== undefined) return { gate: 'continue', opinion }
  }

  // Short-message directive fallback (rule 2 above).
  if (trimmed.length > SHORT_REPLY_LIMIT) return undefined
  const stopWord = STOP_WORDS.find(word => lower.includes(word))
  const continueWord = CONTINUE_WORDS.find(word => lower.includes(word))
  if (stopWord !== undefined && continueWord !== undefined) return undefined
  const directive = stopWord ?? continueWord
  if (directive === undefined) return undefined
  const at = lower.indexOf(directive)
  const prefix = trimmed.slice(0, at).replace(/[\s，,.:：。、]+$/, '')
  if (!SHORT_LEAD_INS.has(prefix)) return undefined
  const trailing = trimmed.slice(at + directive.length).replace(/^[\s，,.:：。、]+/, '')
  if (trailing !== '' && !TRAILING_PARTICLES.test(trailing)) return undefined
  return { gate: stopWord !== undefined ? 'stop' : 'continue', opinion: '' }
}

// ---------------------------------------------------------------------------
// Pure discussion core
// ---------------------------------------------------------------------------

/** Lifecycle phase of one discussion, finer than `RoundtableDiscussion.status`. */
export type RoundtableDriverPhase =
  | 'idle'          // created, round 1 not started
  | 'running'       // a round is executing members right now
  | 'awaitingGate'  // a round settled; waiting for the human gate
...
function roundTopic(baseTopic: string, opinion: string | undefined): string {
  if (opinion === undefined || opinion.trim() === '') return baseTopic
  return `${baseTopic}（人类「继续」新意见：${opinion.trim()}）`
}

export function createRoundtableDriver(
  deps: RoundtableDriverDeps,
  input: RoundtableDriverInput,
): RoundtableDriver {
  const { topic, members, signal } = input
  const seedRounds = input.seedRounds ?? []
  const resumedUnsettled = input.resumedUnsettled ?? false
  const discussion: RoundtableDiscussion = {
    id: input.id ?? RoundtableId(`rt-${crypto.randomUUID()}`),
    roster: members,
    rounds: [...seedRounds],
    status: 'active',
  }
  let phase: RoundtableDriverPhase = seedRounds.length > 0 || resumedUnsettled ? 'awaitingGate' : 'idle'
  ...
  async function runNextRound(roundTopicValue: string): Promise<RoundMinutes> {
    assertActive()
    phase = 'running'
    ...
    try {
      ...
      if (terminal || discussion.status !== 'active') {
        throw new Error(
          `roundtable discussion "${discussion.id}" settled (${discussion.status}) while round ${roundNumber} ran; discarding its minutes`,
        )
      }
  ...
  function start(): Promise<RoundMinutes> {
    if (started) throw new Error('roundtable discussion already started')
    started = true
    return runNextRound(topic)
  }

  function continueRound(opinion?: string): Promise<RoundMinutes> {
  ...
  function fail(reason: Exclude<RoundtableStopReason, 'completed'>): void {
    if (terminal) return
    terminal = true
...
// ---------------------------------------------------------------------------
// Host-persona summarizer (replaces the engine's Task 5 placeholder)
// ---------------------------------------------------------------------------
...
export function renderDriverError(error: unknown): string {
  try {
 ...
 * current round.
 */
export function createHostSummarizer(deps: HostSummarizerDeps): RunRoundDeps['summarize'] {
...
// ---------------------------------------------------------------------------
// Host-agent adapter (discussion-level API)
// ---------------------------------------------------------------------------
...
interface HostEntry {
  core: RoundtableDriver
  info: RoundtableInfo
  controller: AbortController
  /** Effective signal the core runs on (external request signal fused with the local controller). */
  signal: AbortSignal
  settled: RoundtableStopReason | undefined
  /** Round-1 promise; resolves to the minutes, or `undefined` on cancel/error. */
  started: Promise<RoundMinutes | undefined>
}

function assertValidRoster(members: RoundtableMember[], maxMembers = 8): void {
  ...
  if (ids.size !== members.length) throw new RoundtableError('duplicate member id', 'ROSTER_INVALID')
}

export function createRoundtableHostDriver(ctx: Context, deps: RoundtableHostDriverDeps): RoundtableHostDriver {
  const recorder = createRoundtableRecorder(ctx)
  const registry = new Map<RoundtableId, HostEntry>()

  const defaultWriteMarkdown = async (markdown: string, outputFile: string): Promise<void> => {
  ...
  function emitEvent(name: string, ...args: unknown[]): void {
    for (const callback of ctx.events.dispatch('emit', [name, ...args])) {
      try {
        void Promise.resolve((callback as (...a: unknown[]) => unknown)(...args)).catch(error => {
  ...
  function settle(id: RoundtableId, reason: RoundtableStopReason): void {
  ...
  function requireEntry(id: RoundtableId): HostEntry {
    const entry = registry.get(id)
    if (entry === undefined) throw new RoundtableError(`unknown discussion "${id}"`, 'DISCUSSION_UNKNOWN')
    return entry
  }
  ...
  function handleOf(entry: HostEntry): RoundtableDiscussionHandle {
    return {
      id: entry.info.id,
  ...
  return {
    startDiscussion(request) {
      ...
      const entry: HostEntry = { core, info, controller, signal, settled: undefined, started: Promise.resolve(undefined) }
      ...
      entry.started = core.start().catch((error: unknown) => {
        const reason = signal.aborted ? 'cancelled' : 'error'
        if (entry.settled === undefined) settle(id, reason)
    ...
    recover(session) {
      const handles: RoundtableDiscussionHandle[] = []
